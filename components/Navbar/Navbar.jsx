...
const Navbar = () => {
  ...
  // Build the menu items for a given link name
  const getMenuItems = (linkName) => {
    if (linkName === "Academics") return academicsCategories;
    return STATIC_MENU[linkName] || [];
  };
      ...
      >
        ...
        >
                  ...
                  >
                    ...
                    >
                      ...
                      {link.name === "Academics" ? (
                        <>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                              {academicsCategories.map((cat) => (
                                <Link
                                  ...
                                  onClick={closeMenu}
                                  className="group/item cursor-pointer rounded-3xl bg-slate-50 dark:bg-white/5 hover:bg-[#002147] dark:hover:bg-[#002147] transition-colors duration-200 p-5"
                                >
                                  ...
                                  <p className="text-slate-600 dark:text-slate-400 text-xs leading-relaxed group-hover/item:text-white dark:group-hover/item:text-white/90 transition-colors line-clamp-2">
                                    {cat.description || "Explore our high-quality courses in this category."}
                                  </p>
                                </Link>
                              ))}
                            </div>
                          ...
                          <div className="mt-8 pt-6 border-t border-slate-100 dark:border-white/5 flex justify-between items-center">
                            ...
                            <Link
                              href="/courses"
                              onClick={closeMenu}
                              className="h-10 px-8 bg-[#002147] hover:bg-[#003366] text-white text-sm font-bold rounded-2xl transition-all shadow-lg flex items-center justify-center"
                            >
                              View All Courses
                            </Link>
                          </div>
                        ...
                        /* Static menus (Meerza Foundation) */
                        <div className="grid grid-cols-1 gap-2">
                          {STATIC_MENU[link.name]?.map((item, i) => (
                            <Link
                              key={i}
                              href={item.href || "#"}
                              onClick={closeMenu}
                              className="group/item cursor-pointer rounded-3xl bg-slate-50 dark:bg-white/5 hover:bg-[#002147] dark:hover:bg-[#002147] transition-colors duration-200 p-4"
                            >
                              <h4 className="text-slate-900 dark:text-white font-bold text-[14px] mb-1 flex items-center justify-between group-hover/item:text-white dark:group-hover/item:text-white transition-colors">
                                {item.title}
                                <ArrowRight
          ...
          <div className="flex items-center gap-3 z-[110]">
            ...
            <button
              onClick={toggleMenu}
              className={`lg:hidden flex items-center justify-center w-10 h-10 rounded-full relative transition-colors ${isMenuOpen
                  ? "bg-white text-black"
                  : isScrolled || pathname !== "/"
              ...
              <div key={link.name} className="border-b border-slate-100 dark:border-white/5 last:border-0">
                {link.hasMega ? (
                  <button
                    ...
                    className={`w-full py-5 text-xl font-bold flex items-center justify-between text-left transition-colors group ${mobileSubMenu === link.name
                        ? "text-[#002147] dark:text-blue-400"
                        : "text-slate-900 dark:text-white hover:text-[#002147] dark:hover:text-blue-400"
                      }`}
                  >
                    {link.name}
                    <ChevronDown
                      size={20}
                      className={`transition-all duration-300 ${mobileSubMenu === link.name
                          ? "rotate-180 text-[#002147] dark:text-blue-400 scale-110"
                          : "text-slate-400 dark:text-slate-600 group-hover:text-[#002147] dark:group-hover:text-blue-400 group-hover:scale-110"
                        }`}
                    />
                  </button>
                ) : (
                  <Link
                    href={link.href}
                    onClick={closeMenu}
                    className={`block py-5 text-xl font-bold transition-colors group ${pathname === link.href
                        ? "text-[#002147] dark:text-blue-400"
                        : "text-slate-900 dark:text-white hover:text-[#002147] dark:hover:text-blue-400"
                      }`}
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <ArrowRight
                        ...
                        className={`transition-all duration-300 transform ${pathname === link.href
                            ? "text-current translate-x-0"
                            : "text-slate-300 dark:text-slate-700 group-hover:translate-x-1 group-hover:text-[#002147] dark:group-hover:text-blue-400"
                          }`}
                      />
                    </div>
                  </Link>
                      ...
                      <>
                        {academicsCategories.length === 0 ? (
                          [...Array(3)].map((_, i) => (
                            <div
                              key={i}
                              className="p-4 rounded-2xl bg-slate-50 dark:bg-white/5 animate-pulse h-14"
                            />
                          ))
                        ) : (
                          academicsCategories.map((cat) => (
                            ...
                            >
                              ...
                              <ArrowRight
                                size={14}
                                className="text-slate-400 group-hover:text-[#002147] dark:group-hover:text-blue-400 transition-colors"
                              />
                            </Link>
                          ))
                        )}
                        <Link
                          href="/courses"
                          onClick={closeMenu}
                          className="p-4 rounded-2xl bg-[#002147] flex items-center justify-between group active:scale-95 transition-all mt-1"
                        >
                          <span className="text-sm font-bold text-white">View All Courses</span>
                          <ArrowRight size={14} className="text-white" />
                        </Link>
                      </>
                    ) : (
                      getMenuItems(link.name).map((item, i) => (
                        <Link
                          key={i}
                          href={item.href || "#"}
                          onClick={closeMenu}
